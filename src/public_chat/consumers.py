...
class PublicChatConsumer(AsyncJsonWebsocketConsumer):
	...
	async def join_room(self, room_id):
		...
		# send the new users info to the room
	...
	async def connected_users(self, users):
		"""
		Called to send the connected users to the room.
		This users is displayed in the room so other users know how many users are connected to the chat.
		"""
		# Send a message down to the client
		print("PublicChatConsumer: connected_users: users: " + str(users))
		await self.send_json(
			{
...
# 	return PublicChatRoom.objects.get(pk=room_id).users.all()

@database_sync_to_async
def get_connected_users(room_id):
	users = []
	for user in PublicChatRoom.objects.get(pk=room_id).users.all().only("id", "username", "date_joined", "profile_image"):
		users.append(user)
	payload = {}
	if users:
		s = LazyRoomChatUsersEncoder()
		payload['users'] = s.serialize(users)
...
class LazyRoomChatUsersEncoder(Serializer):
	def get_dump_object(self, obj):
		dump_object = {}
		...
		dump_object.update({'natural_timestamp': calculate_timestamp(obj.date_joined)})
		return dump_object
